...
class CaseSearchServiceTest {
    ...
    void shouldSearchCasesByDateProperty() throws JSONException {
        ...
        List<CaseDetails> casesFound = searchService.searchCasesOver112Days();

        assertThat(casesFound).isEqualTo(expectedCases);

        verify(coreCaseDataService).searchCases(eq("UNSPECIFIED_CLAIMS"), queryCaptor.capture());

        String expectedQuery = format("{\"query\":{\"range\":{\"%s\":{\"gte\":\"%s\"}}}}",
                                      property, LocalDateTime.now().plusDays(112));
