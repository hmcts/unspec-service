...
if [[ ${environment} != "prod" && ${environment} != "aat" && ${environment} != "saat" && ${environment} != "demo" && ${environment} != "ithc" && ${environment} != "perftest" && ${environment} != "preview" ]]; then
...
release_definition_output_file=${build_dir}/ccd-civil-unspecified-${environment}.xlsx

mkdir -p ${build_dir}

# build the ccd definition file
export CCD_DEF_CASE_SERVICE_BASE_URL=http://ucmc-service-${environment}.service.core-compute-${environment}.internal
